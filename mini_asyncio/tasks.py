...
class Task(futures._PyFuture):  # Inherit Python Task implementation
    ...
    def __init__(self, coro, *, loop=None):
        ...
        if not coroutines.iscoroutine(coro):    # 传进来的必须为协程对象
            # raise after Future.__init__(), attrs are required for __del__
            # prevent logging for pending task in __del__
            self._log_destroy_pending = False
            raise TypeError(f"a coroutine was expected, got {coro!r}") # {coro!r} means repr(coro)
        ...
        """
            把创建的Task对象的__step包裹成handle对象添加到_ready队列中-尝试立刻进行第一次运行
        """
        self._loop.call_soon(self.__step, context=self._context)  # 把__step包裹成handle对象添加到_ready队列中

        _register_task(self)  # 注册任务 # TODO 为啥要注册任务
    ...
    def __step(self, exc=None):
        """
            往被包装的'协程'对象中send(None)或者throw(exc) 
                即使是stopIteration也是通过这里的throw(exc)来抛出的
            如果自己这一步执行正常但是还没有执行完,那么就再往_ready队列中添加一个handle包裹的自己(__step)
        """
        if self.done():
        ...
        try:
            ...
            """
            如果顺利执行完了,那就去设置好result-同时会设置自己为done
            """
            if self._must_cancel:
            ...
            """
                如果顺利执行并且coro还没有执行完
                blocking表示自身阻塞住了: 需要等待其他任务执行完成先
            """
            ...
            if blocking is not None:
                ...
                if futures._get_loop(result) is not self._loop:
                    ...
                    if result is self:
                        ...
                        """
                            把自己需要等待的任务来出来 设置自己是其回调
                            result 是一个task或future
                        """
...
def ensure_future(coro_or_future, *, loop=None):
    ...
    如果参数是Future，则直接返回。
    """
    if coroutines.iscoroutine(coro_or_future): # 协程对象包装成Task
        if loop is None:
            loop = events.get_event_loop()
        ...
        return task
    elif futures.isfuture(coro_or_future): # future直接返回
        if loop is not None and loop is not futures._get_loop(coro_or_future):
            raise ValueError('loop argument must agree with Future')
        return coro_or_future
    elif inspect.isawaitable(coro_or_future): # awaitable对象包装成协程 
