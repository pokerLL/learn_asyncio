...
class BaseEventLoop(events.AbstractEventLoop):
    ...
    def run_until_complete(self, future):
        ...
        future = tasks.ensure_future(future, loop=self)   # 返回一个Task或者Future对象
        if new_task:
    ...
    def call_soon(self, callback, *args, context=None):
        ...
        handle = self._call_soon(callback, args, context)   # 把callback包裹成handle对象并添加到_ready队列中
    ...
    def _run_once(self):
        ...
        """
            根据一些参数删除已取消的定时任务
            self._scheduled: 定时任务队列  例如 asyncio.sleep(1)
            一些参数:
                len(self._scheduled): 等待执行的延迟呼叫数量
                self._timer_cancelled_count: 已取消的延迟呼叫数量
                _MIN_SCHEDULED_TIMER_HANDLES: 最小等待执行的延迟呼叫数量 100
                _MIN_CANCELLED_TIMER_HANDLES_FRACTION: 已取消的延迟呼叫数量与等待执行的延迟呼叫数量的比例 0.5
        """
        ...
        """
            从select系统调用获取event_list
            如果有准备好的回调或者正在停止 那么久不等待了 直接执行_ready中的handle
            否则最多等待到最快的定时任务到的时间
        """
        timeout = None  # 从select系统调用获取返回值的超时时间
        ...
        """
            处理定时任务 看看有没有定时任务到时间了 如果到了就把他们的callback加入到_raedy队列中
        """

        while self._scheduled:
            handle = self._scheduled[0]
            """
                TODO: end_time 是干嘛的 或者说_clock_resolution是干嘛的
            """
        ...
        for i in range(ntodo):
            ...
            if self._debug:
                ...
                handle._run()       # 执行回调  - 所以_ready队列中应该全部都是handle对象
